...
from agencli.deps import ToolDeps
...
def _get_gitignore_patterns() -> Set[str]:
    """Read a .gitignore patterns if it exists."""
            ...
            if search_for_dirs and output:
                ...
                for line in lines:
                    ...
                    for parent in path.parents:
                        if parent != Path(".") and fnmatch.fnmatch(
                            parent.name, pattern
                        ):
                            dirs.add(str(parent))
    ...
    if exclude_patterns:
        ...
        for idx, pattern_to_exclude in enumerate(exclude_patterns):
            ...
            if "*" in pattern_to_exclude:
                command.extend(f"-name {pattern_to_exclude}")
            elif "/" in pattern_to_exclude:
                command.extend(f"-path ./{pattern_to_exclude}")
            else:
...
async def _find_with_python(
    ...
    """Use Python's os and fnmatch to find files/directories."""
    exclude_patterns = EXCLUDE_DIRS.copy()
    exclude_patterns.update(_get_gitignore_patterns())
