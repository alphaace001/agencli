...
def create_config(config_path: Path) -> Dict:
    ...
    console.print(
        Panel.fit(
            "[bold cyan] Agencli Configuration Setup[/bold cyan]\n\n"
            "Welcome!, Let's setup your configuration.",
            border_style="cyan",
        )
    )
...
def handle_invalid_config(config_path: Path) -> Dict:
    ...
    if choice == "1":
        ...
        raise SystemExit(
            f"Please fix the configuration manually at {config_path} and try again"
        )
...
def run_setup() -> Dict:
    ...
    if config_path.exists():
        ...
        if all(field in config for field in required_fields):
            # Ensure all default field are present
            return ensure_config_structure()
        else:
            console.print(
                "[yellow] Configuration file is missing required fields. [/yellow]"
            )
            return handle_invalid_config(config_path)
